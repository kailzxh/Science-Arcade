...
function HadronFeynmanRotation({ particleToMatch, dataCollection, finishMatching }) {
    ...
    }}>With this orientation, you cannot adjust the vertex to the activated particle</p>)
}
{
    displayCorrectMatch &&
    (<p style={{ color: 'green', position: 'absolute', top: '45%', left: '76%' }}>Well done!</p>)
}

            <div className="feynman-triangle__controls" style={{ width: "100%" }}>
                <div className="feynman-triangle__rotations">
                <button className="main-menu__option space main-menu__option--first" onClick={rotateLeft}>Left Rotation</button>
<button className="main-menu__option space main-menu__option--first" onClick={rotateRight}>Right Rotation</button>

                </div>
                <div className="feynman-triangle__reverse">
                    <button className="main-menu__option space main-menu__option--second" onClick={rotateOpposite}>Reversal</button>
                </div>
                <div className="feynman-triangle__back">
                    <button className="main-menu__option space" onClick={checkForMatch}>Paste</button>
                </div>
            </div>
        </div >
    )
