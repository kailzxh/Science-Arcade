...
function canvasApp() {
    ...
    helpBtn.addEventListener('click',()=>{
       const instructions = `
       How to Play:
       1. **Movement**: Use the 'move forward' command to move your character one step forward.
       2. **Turn**: Use the 'turn' command to change the direction your character is facing (e.g., 'turn left').
       3. **Catch Pokémon**: Once your character is facing the Pokémon, use the 'catch' command to attempt to catch it.
       4. **Walls**: If there is a wall in your path, you will not be able to move forward.
       5. **Objective**: The goal is to move your character towards the Pokémon and catch it.
       `;
       
       alert(instructions);
    });
    settingsBtn.addEventListener('click',function(){
        universalDialog('Settings for the application (reset game, change user info, other settings)');
    });
    accountBtn.addEventListener('click',function(){
        universalDialog('user status (points obtained, levels won)');
    });
